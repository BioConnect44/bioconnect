...
function detectCategory(job) {
  ...
  if (/bioinformatics|computational|data.?scien|machine.?learn/i.test(text))
    return "Bioinformatics";
  if (/clinical|trial|patient|pharmacovig/i.test(text)) return "Clinical";
  if (/research|scientist|r&d|molecular|genomic|biochem|microb/i.test(text))
    return "Research";
  if (/manufactur|production|process|quality|gmp/i.test(text))
    ...
    <AppShell active="/jobs">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: "14px", color: "#6B8A9A" }}>
            Discover biotech opportunities across India
          </p>
        </div>
      ...
      {showAdd && (
        ...
        >
          ...
          >
            Manual jobs have a direct Apply Now link
          </p>
          <div
            style={{
              display: "grid",
          ...
          >
            ...
            <div>
              <label style={L}>Experience</label>
              <input
            ...
            <div>
              ...
              <input
                ...
                style={I}
                placeholder="CRISPR, PCR (comma separated)"
              />
            </div>
            <div style={{ gridColumn: "1/-1" }}>
              <label style={L}>Apply URL *</label>
              <input
            ...
            <div style={{ gridColumn: "1/-1" }}>
              <button
                ...
                disabled={
                  saving || !form.title || !form.company || !form.apply_url
                }
                style={{
                  background: "#14B8A6",
                  color: "#fff",
      ...
      >
        <span style={{ fontSize: "14px", color: "#6B8A9A" }}>
          Showing{" "}
          <strong style={{ color: "#1B2B3A" }}>{filtered.length}</strong>{" "}
          opportunities
        </span>
        {lastUpdated && (
      ...
      >
        ...
        >
          ...
          >
            ...
            {(selectedCats.length > 0 || selectedTypes.length > 0) && (
              <button
                onClick={clearAll}
                style={{
                  fontSize: "12px",
          ...
          {INDUSTRIES.map((cat) => (
            ...
            >
              <input
                ...
                onChange={() =>
                  setSelectedCats(selectedCats.includes(cat) ? [] : [cat])
                }
          ...
          {JOB_TYPES.map((type) => (
            ...
            >
              <input
                ...
                onChange={() =>
                  setSelectedTypes(selectedTypes.includes(type) ? [] : [type])
                }
                style={{ width: 16, height: 16 }}
              />
              <span>{type}</span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {loading ? (
            <div
              style={{ textAlign: "center", padding: "80px", color: "#9CA3AF" }}
            >
              Loading...
            </div>
          ) : filtered.length === 0 ? (
                    ...
                    >
                      {job.url && (
                        <a
                          ...
                          style={{
                            background: isManual ? "#F97316" : "#1B2B3A",
                            color: "#fff",
                            padding: "10px 20px",
                            borderRadius: "10px",
                            fontSize: "14px",
                      ...
                      {isManual && profile?.id === job.posted_by && (
                        <button
                          onClick={() => handleDelete(job._supabaseId)}
                          style={{
                            fontSize: "12px",
                            color: "#EF4444",
                            background: "#FEF2F2",
