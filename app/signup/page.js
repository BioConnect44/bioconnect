  ...
  if (success) return (
    <main style={S.page}><style>{CSS}</style>
      <div style={{ ...S.card, justifyContent: "center", padding: "60px 44px", textAlign: "center" }}>
        ...
        <a href="/login" style={{ color: "#0D9488", fontWeight: 600, fontSize: "14px" }}>Go to Login →</a>
      </div>
    </main>
  );
...
>>>>>>> fa1d966cb33a4fbd1b30a38ad7981ae6c5ca9a1c
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "13px", width: "100%" }}>
            ...
            <button type="submit" disabled={loading} style={S.btn}>{loading ? "Creating account..." : "Log in"}</button>
          ...
          <p style={{ fontSize: "13px", color: "#6B8A9A", marginTop: "14px", textAlign: "center" }}>Already have an account? <a href="/login" style={{ color: "#0D9488", fontWeight: 600 }}>Sign in</a></p>
        </div>
      </div>
    </main>
  );
...
const S = {
  ...
  formPanel: { flex: 1, padding: "44px 40px", display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "center" },
