...
function StudentView({ supabase, profile }) {
  ...
  async function handleDelete(sid, fn) { if (!confirm("Delete?")) return; await supabase.storage.from("course-materials").remove([`${sid}/${fn}`]); await loadNotes(); }
      ...
      <div>
        ...
        <div style={{ background: "linear-gradient(135deg, #132D35 0%, #1B4A5A 100%)", borderRadius: "20px", padding: "28px 32px", marginBottom: "28px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ position: "relative", width: 100, height: 100, flexShrink: 0 }}>
            <svg width="100" height="100" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="40" fill="none" stroke="#14B8A6" strokeWidth="8" strokeLinecap="round" strokeDasharray={`${2*Math.PI*40*0.75} ${2*Math.PI*40*0.25}`} strokeDashoffset={2*Math.PI*40*0.25} transform="rotate(-90 50 50)"/>
            </svg>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "28px" }}>
          <div style={{ background: "#FFF3E8", borderRadius: "14px", padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid #FFD4A3" }}>
            <div><p style={{ fontSize: "15px", fontWeight: 700, color: "#1B2B3A", marginBottom: "4px" }}>Genetics PYQs</p><p style={{ fontSize: "12px", color: "#6B8A9A" }}>10 MCQs • <span style={{ color: "#F97316", fontWeight: 600 }}>+100 XP</span></p></div>
          ...
          <div style={{ background: "#F3F0FF", borderRadius: "14px", padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid #DDD6FE" }}>
            <div><p style={{ fontSize: "15px", fontWeight: 700, color: "#1B2B3A", marginBottom: "4px" }}>Timed Mock Exam</p><p style={{ fontSize: "12px", color: "#6B8A9A" }}>20 MCQs • <span style={{ color: "#8B5CF6", fontWeight: 600 }}>+100 XP</span></p></div>
        ...
        {isResearcher && (
          <div style={{ background: "#F0FCFB", border: "1.5px dashed #14B8A6", borderRadius: "14px", padding: "18px 22px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <div><p style={{ fontSize: "15px", fontWeight: 600, color: "#1B2B3A" }}>Share your expertise</p><p style={{ fontSize: "13px", color: "#6B8A9A" }}>Upload your research notes or course materials</p></div>
              ...
              <div key={sub.id} style={{ background: "#fff", borderRadius: "14px", border: "1px solid #E2EEF0", overflow: "hidden" }}>
                <button onClick={() => setOpenSubject(isOpen ? null : sub.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: "14px", padding: "16px 20px", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}>
                  ...
                  <div style={{ width: "120px", margin: "0 12px" }}>
                    <div style={{ height: 4, background: "#E2EEF0", borderRadius: "4px" }}><div style={{ height: 4, width: "30%", background: sub.color, borderRadius: "4px" }}></div></div>
                  </div>
                ...
                {isOpen && (
                  <div style={{ padding: "0 20px 20px", borderTop: "1px solid #F0F7F8" }}>
                    {pdfs.length === 0 ? <p style={{ fontSize: "13px", color: "#9CA3AF", padding: "14px 0 8px" }}>No notes uploaded yet.</p> : (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        <div style={{ background: "#fff", borderRadius: "16px", padding: "20px", border: "1px solid #E2EEF0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
            ...
            <div><span style={{ fontSize: "28px", fontWeight: 800, color: "#F97316" }}>14</span><span style={{ fontSize: "14px", color: "#1B2B3A", fontWeight: 600 }}> Day Study Streak!</span></div>
          </div>
          <div style={{ display: "flex", gap: "6px" }}>
            {["M","T","W","T","F","S","S"].map((d, i) => (
              <div key={i} style={{ width: 32, height: 32, borderRadius: "50%", background: i < 4 ? "#14B8A6" : "#E2EEF0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 700, color: i < 4 ? "#fff" : "#9CA3AF" }}>{d}</div>
            ))}
          </div>
        </div>
        <div onClick={() => {}} style={{ background: "#14B8A6", borderRadius: "16px", padding: "20px", cursor: "pointer" }}>
          <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.7)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "1px" }}>DAILY BIO-MINUTE</span>
          <p style={{ fontSize: "16px", fontWeight: 700, color: "#fff", margin: "10px 0 16px", lineHeight: "1.4" }}>What enzyme unwinds DNA during replication?</p>
          <button style={{ background: "rgba(255,255,255,0.2)", color: "#fff", border: "1.5px solid rgba(255,255,255,0.4)", padding: "8px 18px", borderRadius: "8px", fontSize: "13px", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Tap to flip</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "14px", marginBottom: "28px" }}>
        {[
          ...
          <div key={s.label} style={{ background: "#fff", borderRadius: "14px", padding: "18px 20px", border: "1px solid #E2EEF0", display: "flex", gap: "12px", alignItems: "center" }}>
            ...
            <div><p style={{ fontSize: "20px", fontWeight: 700, color: "#1B2B3A" }}>{s.value}</p><p style={{ fontSize: "12px", color: "#9CA3AF" }}>{s.label}</p></div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
                ...
                <div key={sub.id} style={{ background: "#fff", borderRadius: "14px", border: "1px solid #E2EEF0", overflow: "hidden" }}>
                  ...
                  {isOpen && (
                    <div style={{ padding: "0 18px 18px", borderTop: "1px solid #F0F7F8" }}>
                      {pdfs.length === 0 ? <p style={{ fontSize: "13px", color: "#9CA3AF", padding: "12px 0 8px" }}>No files yet</p> : (
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            ...
            <button onClick={() => setShowAddMCQ(!showAddMCQ)} style={{ background: "#F97316", color: "#fff", border: "none", padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{showAddMCQ ? "Cancel" : "+ Add MCQ"}</button>
          </div>
          ...
          {showAddMCQ && (
            <div style={{ background: "#fff", borderRadius: "14px", padding: "20px", border: "1px solid #E2EEF0", marginBottom: "14px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <div><label style={L}>Question *</label><textarea value={mcqForm.question} onChange={e => setMcqForm({...mcqForm, question: e.target.value})} rows={2} style={{...I, resize: "vertical"}} placeholder="Enter your MCQ question..."/></div>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  <div style={{ flex: 1 }}><label style={L}>XP Reward</label><input value={mcqForm.xp} onChange={e => setMcqForm({...mcqForm, xp: e.target.value})} style={I} placeholder="20"/></div>
                </div>
                <button onClick={() => { if (!mcqForm.question || mcqForm.options.some(o => !o)) { alert("Fill all fields"); return; } alert("MCQ saved! (Connect to database to persist)"); setShowAddMCQ(false); setMcqForm({ subject: "genetics", question: "", options: ["","","",""], answer: 0, xp: "20" }); }} style={{ background: "#F97316", color: "#fff", border: "none", padding: "11px 20px", borderRadius: "10px", fontSize: "14px", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Save MCQ</button>
              </div>
            </div>
          )}
...
export default function LearningPage() {
  ...
  return (
    <AppShell active="/learning">
      {loading ? <div style={{ textAlign: "center", padding: "100px", color: "#9CA3AF" }}>Loading...</div> :
