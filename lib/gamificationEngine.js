export const BADGE_CATALOG = [
  ...
  },
  {
    ...
    icon: "📅",
    theme: { bg: "bg-white", border: "border-[#E2E8F0]", text: "text-[#94A3B8]", activeBg: "#F5F3FF", activeBorder: "#DDD6FE", activeText: "#7C3AED" },
    xpReward: 75,
    isUnlocked: (stats) => (stats.eventsRegistered || stats.events_registered || 0) >= 1,
    checkUnlock: (stats) => (stats.eventsRegistered || stats.events_registered || 0) >= 1,
    getProgress: (stats) => ({ current: Math.min(stats.eventsRegistered || stats.events_registered || 0, 1), total: 1 })
  },
  {
    id: "research-star",
    title: "Research Star",
    description: "Read 5 papers",
    icon: "⭐",
    theme: { bg: "bg-white", border: "border-[#E2E8F0]", text: "text-[#94A3B8]", activeBg: "#F0FDFA", activeBorder: "#99F6E4", activeText: "#0D9488" },
    xpReward: 150,
    isUnlocked: (stats) => (stats.papersRead || stats.papersReadCount || 0) >= 5,
    checkUnlock: (stats) => (stats.papersRead || stats.papersReadCount || 0) >= 5,
    getProgress: (stats) => ({ current: Math.min(stats.papersRead || stats.papersReadCount || 0, 5), total: 5 })
  },
  {
    id: "fast-learner",
    title: "Fast Learner",
    ...
    icon: "🚀",
    theme: { bg: "bg-white", border: "border-[#E2E8F0]", text: "text-[#475569]", activeBg: "#F1F5F9", activeBorder: "#CBD5E1", activeText: "#475569" },
    xpReward: 300,
  ...
  },
  {
    ...
    icon: "⚡",
    theme: { bg: "bg-white", border: "border-[#E2E8F0]", text: "text-[#D97706]", activeBg: "#FFFBEB", activeBorder: "#FDE68A", activeText: "#D97706" },
    xpReward: 50,
    isUnlocked: (stats) => (stats.totalXp || stats.total_xp || 0) >= 100,
    checkUnlock: (stats) => (stats.totalXp || stats.total_xp || 0) >= 100,
    getProgress: (stats) => ({ current: Math.min(stats.totalXp || stats.total_xp || 0, 100), total: 100 })
  },
  {
    id: "rising-scholar",
    ...
    icon: "🎓",
    theme: { bg: "bg-white", border: "border-[#E2E8F0]", text: "text-[#059669]", activeBg: "#F0FDF4", activeBorder: "#A7F3D0", activeText: "#059669" },
    xpReward: 150,
    isUnlocked: (stats) => (stats.totalXp || stats.total_xp || 0) >= 500,
    checkUnlock: (stats) => (stats.totalXp || stats.total_xp || 0) >= 500,
    getProgress: (stats) => ({ current: Math.min(stats.totalXp || stats.total_xp || 0, 500), total: 500 })
  },
  {
    id: "biotech-master",
    title: "Biotech Master",
    description: "Reach 1000 total XP",
    icon: "👑",
    theme: { bg: "bg-white", border: "border-[#E2E8F0]", text: "text-[#7C3AED]", activeBg: "#FAF5FF", activeBorder: "#E9D5FF", activeText: "#7C3AED" },
    xpReward: 300,
    isUnlocked: (stats) => (stats.totalXp || stats.total_xp || 0) >= 1000,
    checkUnlock: (stats) => (stats.totalXp || stats.total_xp || 0) >= 1000,
    getProgress: (stats) => ({ current: Math.min(stats.totalXp || stats.total_xp || 0, 1000), total: 1000 })
  }
];
...
export const recordUserAction = async (userId, actionType, payload = {}, supabase = null) => {
  ...
  switch (actionType) {
    ...
    case "READ_PAPER":
      ...
      break;
    case "ENROLL_COURSE":
      stats.courses_enrolled = (stats.courses_enrolled || 6) + 1;
      xpAdded = 50;
      break;
    case "COMPLETE_QUIZ":
  ...
  const evalStats = {
    ...
    coursesEnrolled: stats.courses_enrolled || 6,
  ...
  try {
    if (typeof window !== "undefined") {
      if (stats.streak_days !== undefined) localStorage.setItem("bioconnect_streak", String(stats.streak_days));
      if (stats.papers_read !== undefined) localStorage.setItem("bioconnect_papers_read", String(stats.papers_read));
      if (stats.courses_enrolled !== undefined) localStorage.setItem("bioconnect_courses_enrolled", String(stats.courses_enrolled));
      if (stats.unlocked_badge_ids) localStorage.setItem("bioconnect_unlocked_badges", JSON.stringify(stats.unlocked_badge_ids));
    }
  } catch (e) {}
  ...
  if (supabase && userId) {
    try {
      await supabase.from("user_gamification").upsert({
        ...
        courses_enrolled: stats.courses_enrolled || 6,
        quizzes_completed: stats.quizzes_completed,
        notes_accessed: stats.notes_accessed,
        total_xp: stats.total_xp,
        unlocked_badge_ids: stats.unlocked_badge_ids || [],
        updated_at: new Date().toISOString()
      });
    } catch (e) {}
  }
